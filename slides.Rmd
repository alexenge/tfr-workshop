...
# Show figure
_ = plt.subplots_adjust(wspace=3.)
_ = fig.tight_layout()
_ = plt.show()
```
...
```{python, echo=TRUE, out.width="80%"}
# Spectrum for all channels, restricted to 0--30 Hz
...
**Approach:** Apply the Fourier transform to a short time window and move this over time points in the epoch $\rightarrow$ Short-term Fourier transform (STFT), Morlet wavelet convolution
...
_ = plt.colorbar(im, label='Power (µV²)')
_ = ax.set_xlabel('Time (s)')
...
- Should account for the $\frac{1}{F}$ scaling $\rightarrow$ Divisive baseline

    - Subtract + divide by mean baseline: Percent signal change

    - Divide by mean baseline + take logarithm: Decibel

- Baseline should end *before* rather tha *at* stimulus onset

    - Prevents low-freq activity to "smear" into the baseline
...
# Plot power at one channel, averaged across all epochs
tfr_ave = tfr.average()
...
res <- pipeline$group_pipeline(
  ...
  perform_tfr = TRUE,
  tfr_freqs = seq(6, 30, 2),
  tfr_cycles = seq(3, 15, 1),
...
$\rightarrow$ Then perform linear mixed-effects model analysis on single trial power

## Time-frequency analysis with the pipeline

For exploratory analyses $\rightarrow$ Cluster-based permutation test
...
$\rightarrow$ Creates a new data frame with cluster-level $p$ values for all time samples, channels, and frequencies

## General considerations

- New parameters to be taken care of:

    - Design $\rightarrow$ Longer inter-trial interval

    - Wavelet frequencies + number of cycles $\rightarrow$ TF tradeoff

    - Baseline correction window and method

    - Interpretation: narrow-band vs. broad-band, oscillation vs. rate of change
